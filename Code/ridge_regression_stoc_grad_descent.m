...
for k=1:niter
    tic
    alpha=1e-1/k;
    w1(:,k+1)=w1(:,k)-2*alpha*((x_train(:,indperm(indcount))-xbar)*(w1(:,k)'*(x_train(:,indperm(indcount))-xbar)-(y_train(indperm(indcount)))-ybar)+lambda*w1(:,k));
    %     training error
    b(k+1)=ybar-w1(:,k+1)'*xbar;
    y_pred_train_real=x_train'*w1(:,k+1)+b(k+1);
    y_pred_train=(round(y_pred_train_real))';
...
RR(1:niter,:)=[RMSE_train(2:end)' RMSE_test(2:end)' cumsum(elapsed)'];
RR_class(1:niter,:)=[perc_e_train(2:end)' perc_e_test(2:end)' cumsum(elapsed)'];

save('RidgeNetf');
% save('Ridge_mv10k');
save('RR','RR','RR_class');
% save('RR_mv10k','RR','RR_class');